...
One recorder for every algorithm whose subject is a flat array: all six sorts
plus both searches. Emitting a single schema means the frontend has exactly one
array renderer to maintain.

A step:

    {
      "array":    [42, 17, 68, 9],  # the array AFTER this operation
      "ids":      [0, 1, 2, 3],     # stable identity per slot (drives animation)
      "type":     "compare",        # see OPERATIONS below
      "indices":  [2, 3],           # main-array slots this operation touched
      "active":   3,                # the key / pivot / current element, or None
      "sorted":   [3],              # slots known to hold their final value
      "pointers": {"low": 0, "high": 3, "mid": 1},
      "auxLeft":  null,             # merge buffer halves, when in play
      "auxRight": null,
      "message":  "Compare 68 and 9",
      "compared": [2, 3],           # legacy alias for `indices`
      "swapped":  False             # legacy flag
    }

OPERATIONS
    select     focus an element (key, pivot, current minimum)
    compare    read two slots, never mutates
    swap       exchange two slots
    shift      insertion-sort shift: one element right, key into the gap
    insert     confirm the key landed
    write      overwrite a slot from the merge buffer
    divide     merge sort split marker
    probe      search inspects a slot
    found      search succeeded
    notfound   search exhausted the space
    sorted     slots reached their final position
    done       finished

WHY `ids` MATTERS
    The array says what the values are, not which element moved. Carrying a
    stable id per element, permuted exactly as the values are, lets the frontend
    key each bar by id so Framer Motion animates the same DOM node from its old
    slot to its new one. Without it, bars remount and teleport.
"""
...
class ArrayStepRecorder:
    ...
    def set_pointers(self, **named):
        """Named index markers drawn under the bars: low/high/mid, i/j/pivot."""
    ...
    def shift_right(self, src, key_value, key_id, message=None):
        ...
        hole and the key drops into the slot it vacated, so the key visibly walks
        left one position per shift instead of jumping to its destination.
        """
    ...
    # ------------------------------------------------------------------ #
    # auxiliary buffer (merge sort)
    # ------------------------------------------------------------------ #
    # Elements are MOVED into the buffer, not copied. The vacated array slots
    # become holes (value None, negative placeholder id) until the merge writes
    # them back.
    #
    # That matters for more than tidiness: if elements were copied, the same id
    # would exist in the array and the buffer at once, and ids are React keys.
    # Duplicate keys break the animation. Moving keeps every id unique at every
    # instant, which also lets the frontend give a bar and its buffer cell the
    # same Framer Motion layoutId, so a value physically flies from the buffer
    # back into the array.
